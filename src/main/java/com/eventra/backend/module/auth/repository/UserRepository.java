...
public interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Query("""
        ...
        AND u.id <> :excludeId
        AND (LOWER(u.fullName) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(u.email) LIKE LOWER(CONCAT('%', :query, '%')))
    """)
    java.util.List<User> searchUsers(@Param("query") String query, @Param("excludeId") UUID excludeId, Pageable pageable);
}
